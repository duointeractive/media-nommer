...
class AWSJobStateBackend(object):
    ...
    def _get_sdb_job_state_domain(self):
        """
        ...
        if not self.__aws_sdb_job_state_domain:
            self.__aws_sdb_job_state_domain = self._get_sdb_connection().create_domain(
    ...
    def _get_sqs_new_job_queue(self):
        """
        ...
        if not self.__aws_sqs_new_job_queue:
            self.__aws_sqs_new_job_queue = self._get_sqs_connection().create_queue(
                settings.SQS_NEW_JOB_QUEUE_NAME)
        return self.__aws_sqs_new_job_queue

    def _get_sqs_state_change_queue(self):
        """
        Lazy-loading of the SQS boto queue. Refer to this instead of
        ...
        if not self.__aws_sqs_state_change_queue:
            self.__aws_sqs_state_change_queue = self._get_sqs_connection().create_queue(
    ...
    def get_job_object_from_id(self, unique_id):
        ...
        """
        item = self._get_sdb_job_state_domain().get_item(unique_id)
        if item is None:
    ...
    def wipe_all_job_data(self):
        ...
        """
        try:
            self._get_sdb_connection().delete_domain(settings.SIMPLEDB_JOB_STATE_DOMAIN)
            self._get_sqs_new_job_queue().clear()
        ...
        self.__aws_sdb_job_state_domain = None
        # Reset our local cache of the boto SQS queue object.
        self.__aws_sqs_new_job_queue = None
    ...
    def get_unfinished_jobs(self):
        ...
        results = self._get_sdb_job_state_domain().select(query_str)
    ...
    def pop_new_jobs_from_queue(self, num_to_pop):
        ...
        """
        return self._pop_jobs_from_queue(self._get_sqs_new_job_queue(),
                                         num_to_pop,
    ...
    def pop_state_changes_from_queue(self, num_to_pop):
        ...
        """
        return self._pop_jobs_from_queue(self._get_sqs_state_change_queue(),
                                         num_to_pop,
                                         visibility_timeout=3600)
